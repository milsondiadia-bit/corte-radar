...
SISTEMA = """You analyze video posts from OSINT and geopolitics accounts on X (MarioNawfal, Osint613, clashreport, nexta_tv).

Decide whether the video is a CLIP of a public figure SPEAKING - an interview, press conference, speech, address, parliamentary session, hearing, testimony, podcast, doorstep remarks, or statement to reporters. The person on camera must be talking, and there must be a longer original recording somewhere.

NOT a speaking clip: combat or drone footage, aftermath of strikes, CCTV or dashcam video, protests, explosions, military hardware, satellite imagery, narrated news packages with no on-camera speaker, memes, animations, ads, AI-generated or edited compilations with voiceover.
...
def _montar_pergunta(post, transcricao, falante):
    ...
    if transcricao:
        ...
        partes.append("(No transcript available - judge from the caption alone and lower your confidence accordingly.)")
    return "\n\n".join(p for p in partes if p)


def _gemini(pergunta, cfg):
    chave = os.environ["GEMINI_API_KEY"]
    modelo = cfg["ia"]["modelo"].replace("models/", "")
    ...
    for versao in ("v1beta", "v1"):
        url = f"https://generativelanguage.googleapis.com/{versao}/models/{modelo}:generateContent"
        ...
        for tentativa in range(4):
            ...
            if r.status_code in (401, 403, 404):
                ultimo_erro = f"{r.status_code} em {versao}: {r.text[:150]}"
                break
            if not r.ok:
                ultimo_erro = f"{r.status_code}: {r.text[:150]}"
                break
            ...
            try:
                ...
                raise RuntimeError(f"resposta inesperada: {str(dados)[:300]}")

    raise RuntimeError(f"Gemini falhou ({ultimo_erro}). Modelo: {modelo}")
